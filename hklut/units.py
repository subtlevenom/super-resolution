...
# HDTB
class HDBPlusUnit(nn.Module):
    rot_dict = {'h': [0, 1, 2, 3], 'd': [0, 1, 2, 3], 't': [0, 1, 2, 3], 'b': [0, 1, 2, 3]}
    pad_dict = {'h': (0, 3, 0, 3), 'd': (0, 3, 0, 3), 't': (0, 3, 0, 3), 'b': (0, 3, 0, 3)}
    avg_factor = 4.

    def __init__(self, ktype, nf=64, upscale=4, act=nn.ReLU):
        super(HDBPlusUnit, self).__init__()
        self.ktype = ktype
        self.upscale = upscale
        self.act = act()
    ...
    def dconv_forward(self, x):
        ...
        if self.ktype == 'h':
            x = torch.cat([x[:, :, 0, 0], x[:, :, 0, 1], x[:, :, 0, 2], x[:, :, 0, 3]], dim=1)
        elif self.ktype == 'd':
            x = torch.cat([x[:, :, 0, 0], x[:, :, 1, 1], x[:, :, 2, 2], x[:, :, 3, 3]], dim=1)
        elif self.ktype == 't':
            x = torch.cat([x[:, :, 0, 0], x[:, :, 2, 1], x[:, :, 3, 1], x[:, :, 3, 2]], dim=1)
        elif self.ktype == 'b':
            x = torch.cat([x[:, :, 0, 0], x[:, :, 1, 2], x[:, :, 1, 3], x[:, :, 2, 3]], dim=1)
        else:
            raise AttributeError
    ...
    def forward(self, x_in):
        ...
        x = self.conv6(x)
        x = self.pixel_shuffle(x)
        x = x.reshape(B, C, self.upscale * (H - 3), self.upscale * (W - 3))
        return torch.tanh(x)

    def get_lut_input(self, input_tensor):

        input_tensor_dil = torch.zeros((input_tensor.shape[0], input_tensor.shape[1], 4, 4), dtype=input_tensor.dtype)
        if self.ktype == 'h': # green
            input_tensor_dil[:, :, 0, 0] = input_tensor[:, :, 0]
            input_tensor_dil[:, :, 0, 1] = input_tensor[:, :, 1]
            input_tensor_dil[:, :, 0, 2] = input_tensor[:, :, 2]
            input_tensor_dil[:, :, 0, 3] = input_tensor[:, :, 3]
        elif self.ktype == 'd': # red
            input_tensor_dil[:, :, 0, 0] = input_tensor[:, :, 0]
            input_tensor_dil[:, :, 1, 1] = input_tensor[:, :, 1]
            input_tensor_dil[:, :, 2, 2] = input_tensor[:, :, 2]
            input_tensor_dil[:, :, 3, 3] = input_tensor[:, :, 3]
        elif self.ktype == 't':
            input_tensor_dil[:, :, 0, 0] = input_tensor[:, :, 0]
            input_tensor_dil[:, :, 2, 1] = input_tensor[:, :, 1]
            input_tensor_dil[:, :, 3, 1] = input_tensor[:, :, 2]
            input_tensor_dil[:, :, 3, 2] = input_tensor[:, :, 3]
        elif self.ktype == 'b':
            input_tensor_dil[:, :, 0, 0] = input_tensor[:, :, 0]
            input_tensor_dil[:, :, 1, 2] = input_tensor[:, :, 1]
            input_tensor_dil[:, :, 1, 3] = input_tensor[:, :, 2]
            input_tensor_dil[:, :, 2, 3] = input_tensor[:, :, 3]
        else:
            raise AttributeError
