...
def numpy_bit_plane_slicing(x, bit_mask='11110000'):
    m = int(bit_mask, 2)
    masks = [m, 255 - m]
    images = []
    for mask in masks:
        images.append((x.astype(np.uint8) & mask).astype(np.float32))
    return images
...
def bit_plane_slicing(x, bit_mask='11110000'):
    ...
    images = []
    for mask in masks:
        images.append((x.type(torch.LongTensor) & mask).type(
            torch.FloatTensor).to(x.device))
    return images
...
def _ycbcr2rgb(im):
    ...
    cr  = im[:,:,2] - 128
    # R
    rgb[:,:,0] = y * 1.164 + 1.596 * cr
    # G
    rgb[:,:,1] = y * 1.164 - 0.392 * cb - 0.813 * cr
    # B
    rgb[:,:,2] = y * 1.164 + 2.017 * cb

    return np.clip(rgb, 0, 255)


def _load_img_array(path,
                    color_mode='RGB',
                    channel_mean=None,
                    modcrop=[0, 0, 0, 0]):
